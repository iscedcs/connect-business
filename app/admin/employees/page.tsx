...
export default function Employees(props: EmployeesProps) {
	...
	const [teamO, setTeamO] = useState<any>([]);
	const [teamW, setTeamW] = useState<any>([]);
	...
	React.useEffect(() => {
		const getData = async () => {
			...
			if (response.status !== 200) {
				...
				const tmO = serverData.data.team_members.onboarded;
				const tmW = serverData.data.team_members.waitlisted;
				setTeamO(tmO);
				setTeamW(tmW);
				// console.log(tmO, tmW);
		...
		getData();
	}, [confirmIsLoading, profileDeleteOpen]);
